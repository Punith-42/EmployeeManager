...
function updateSortIndicators() {
  const headers = {
    name: document.getElementById('headerName'),
    role: document.getElementById('headerRole'),
    salary: document.getElementById('headerSalary'),
  };
  for (const field in headers) {
    headers[field].textContent = field.charAt(0).toUpperCase() + field.slice(1);
    if (currentSortBy === field) {
      headers[field].textContent += currentSortDir === "asc" ? " ▲" : " ▼";
    }
  }
}
